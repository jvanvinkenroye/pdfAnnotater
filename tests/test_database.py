...
class TestCreateDocument:
    ...
    def test_create_document_returns_uuid(self, db):
        doc_id = db.create_document("test.pdf", "/path/test.pdf", 5)
    ...
    def test_create_multiple_documents(self, db):
        id1 = db.create_document("a.pdf", "/a.pdf", 1)
        id2 = db.create_document("b.pdf", "/b.pdf", 2)
        assert id1 != id2
...
class TestGetDocument:
    ...
    def test_get_existing_document(self, db):
        doc_id = db.create_document("test.pdf", "/path/test.pdf", 3)
        doc = db.get_document(doc_id)
...
class TestDeleteDocument:
    ...
    def test_delete_existing_document(self, db):
        doc_id = db.create_document("test.pdf", "/path/test.pdf", 2)
...
class TestAnnotations:
    ...
    def test_get_all_annotations_empty_doc(self, db):
        doc_id = db.create_document("test.pdf", "/path/test.pdf", 1)
        annotations = db.get_all_annotations(doc_id)
        assert annotations == []
